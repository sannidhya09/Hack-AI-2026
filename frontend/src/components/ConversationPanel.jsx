    ...
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {!aiSpeaking && tripStarted && listenMode === "active" && (
            <div className="flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />
              <span className="text-[10px] font-500 text-green-500">Listening...</span>
            </div>
          )}
          {!aiSpeaking && tripStarted && listenMode === "wake" && (
            <div className="flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full bg-gray-200" />
              <span className="text-[10px] font-500 text-gray-300">Say "Nina"</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-3 max-h-56 overflow-y-auto space-y-2">
        {conversation.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <p className="text-sm text-gray-400">Say "Nina" to talk to CoDriver</p>
            <p className="text-xs text-gray-300 mt-1">Or just drive — safety alerts fire automatically</p>
          </div>
        ) : (
          conversation.map((msg, i) => {
            const isAI  = msg.sender === "CoDriver";
            const style = isAI ? (TYPE_STYLES[msg.type] || TYPE_STYLES.normal) : TYPE_STYLES.driver;
            return (
              <div key={i} className={`rounded-2xl p-3 border animate-slide-up ${style}`}>
      ...
      <div className="px-4 py-2 border-t border-gray-50">
        <p className="text-[10px] text-gray-300 text-center">
          Say "Nina" to talk • "Nina play music" • Safety alerts always on
        </p>
      </div>
    </div>
  );
