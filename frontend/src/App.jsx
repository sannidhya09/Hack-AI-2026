...
// Give backend enough time for Gemini + ElevenLabs on longer responses
...
const PROACTIVE_MS  = 300000; // 5 min
...
export default function App() {
  ...
  // Single live ref — avoids stale closures in all intervals/callbacks
  const live = useRef({
  ...
  useEffect(() => {
    ...
    rec.onend    = () => {
      setListening(false);
      if (!live.current.aiSpeaking) setTimeout(() => { try { rec.start(); } catch {} }, 500);
    };
    rec.onerror  = (e) => {
  ...
  const handleDriverSpoke = (text) => {
    if (!live.current.tripStarted) return; // ignore before trip starts
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-8 max-w-md mx-auto">
        ...
        <div className="w-full space-y-3 mb-8">
          {["🎙️ Listens to your voice", "📡 Reads hardware sensors", "🧠 Powered by Gemini 2.5", "🚨 Emergency crash detection"]
            .map((f, i) => (
  ...
  return (
    <div className="min-h-screen bg-white max-w-md mx-auto relative">
      ...
      <div className="pb-6">
        <Header connected={connected} listening={listening} aiSpeaking={aiSpeaking} micVolume={micVolume} />
        <div className="px-4 space-y-3">
          ...
          <ConversationPanel conversation={conversation} aiSpeaking={aiSpeaking}
            listening={listening} micVolume={micVolume} />
        </div>
      </div>
    </div>
  );
